        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {serviceCards.map((card, i) => (
            <motion.div
              ...
              className={`group ${CLAY_CLASSES.cardCream} p-7 sm:p-8 flex flex-col justify-between h-full relative transition-all duration-300 hover:shadow-2xl hover:border-charcoal-brand`}
            >
              <div>
                {/* Header Icon & Index */}
                <div className="flex justify-between items-start mb-6">
                  <Magnetic strength={0.3}>
                    <div className={`w-14 h-14 flex items-center justify-center rounded-2xl ${card.badgeClass} group-hover:rotate-6 transition-transform`}>
                      {card.icon}
                    </div>
                  </Magnetic>

                  <span className="font-mono text-xs font-bold text-charcoal-brand/30 group-hover:text-emerald-brand transition-colors">
                    0{i + 1} // 03
                  </span>
                </div>
                ...
                <div className="border-t border-dashed border-charcoal-brand/15 pt-5 mb-6">
                  ...
                  <ul className="flex flex-col gap-2">
                    {card.deliverables.map((deliv, dIdx) => (
                      <li key={dIdx} className="flex items-center gap-2 text-xs font-mono text-charcoal-brand/85 font-semibold group-hover:translate-x-1 transition-transform" style={{ transitionDelay: `${dIdx * 40}ms` }}>
                ...
                <div className="border-t border-dashed border-charcoal-brand/15 pt-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {card.tools.map((tool) => (
                      <span
                        key={tool}
                        className="text-[10px] font-mono font-bold px-2.5 py-1 bg-charcoal-brand/5 text-charcoal-brand border border-charcoal-brand/15 rounded-lg group-hover:bg-mustard-brand/20 group-hover:border-mustard-brand/40 transition-colors"
                      >
                        {tool}
                      </span>
                    ))}
